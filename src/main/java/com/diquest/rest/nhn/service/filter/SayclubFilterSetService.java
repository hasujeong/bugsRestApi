...
public class SayclubFilterSetService {
    ...
    public FilterSet[] parseFilter(String paramFilter, FilterFieldParseResult filterFieldParseResult, String collection) throws InvalidParameterException {
...
//        filters.addAll(makeTriggerFilterFields(TriggerFieldService.getInstance().getTriggerFieldNames(params), filterFieldParseResult));
        filters.addAll(makeParamFilterFields(paramFilter, collection)); 
//        filters.addAll(makeFilterFields(filterFieldParseResult, collection));
    ...
    private List<FilterSet> makeParamFilterFields(String paramFilter, String collection) throws InvalidParameterException {
        List<FilterSet> filters = new ArrayList<>();
        String[] paramsFiters;
        
        if(!paramFilter.equalsIgnoreCase("")) {
        	if(paramFilter.indexOf("\\|") > -1) {
        		paramsFiters = paramFilter.split("\\|");
        		
        		for(int i=0 ; i < paramsFiters.length ; i++) {
        			String key = paramsFiters[i].split("=")[0].toUpperCase();
        			String value = paramsFiters[i].split("=")[1];
        			
        			filters.add(new FilterSet(Protocol.FilterSet.OP_MATCH, key, value));
    ...
    private List<FilterSet> makeFilterFields(FilterFieldParseResult filterFieldParseResult, String collection) throws InvalidParameterException {
        ...
        for (FilterNormalValueResult tnf : normalFields) {
        	 for (FieldValue v : tnf.getValues()) {
        		 ...
        		 System.out.println("===============> " + tnf.getFieldName() + " <==========> " + v.getValue());
        	 }
        }
